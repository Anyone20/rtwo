...
download_RealM(){
        echo -e "#############################################################"
        echo -e "#                     请选择下载版本:                       #"  
        echo -e "#    1.realm-1_4（原作者最后一版）      2.realm-1_5-aarch64-linux-gnu      #"
        echo -e "#    3.realm-1_5-aarch64-linux-musl  4.realm-1_5-arm-linux-musleabihf   #"
        echo -e "#############################################################"
        read -p "请选择(默认为2): " version
        [[ -z ${version} ]] && version="2"
        if [[ ${version} == [1] ]]; then  
            mkdir /etc/realm
            wget -N --no-check-certificate https://raw.githubusercontent.com/Anyone20/rtwo/main/realm1.4 && chmod +x realm1.4 && mv realm1.4 /etc/realm/realm
        elif [[ ${version} == [2] ]]; then  
            mkdir /etc/realm
            wget -N --no-check-certificate https://raw.githubusercontent.com/Anyone20/rtwo/main/realm-1_5-aarch64-linux-gnu && chmod +x realm-1_5-aarch64-linux-gnu && mv realm-1_5-aarch64-linux-gnu /etc/realm/realm
        elif [[ ${version} == [3] ]]; then  
            mkdir /etc/realm
            wget -N --no-check-certificate https://raw.githubusercontent.com/Anyone20/rtwo/main/realm-1_5-aarch64-linux-musl && chmod +x realm-1_5-aarch64-linux-musl && mv realm-1_5-aarch64-linux-musl /etc/realm/realm
        else
            mkdir /etc/realm
            wget -N --no-check-certificate https://raw.githubusercontent.com/Anyone20/rtwo/main/realm-1_5-arm-linux-musleabihf && chmod +x realm-1_5-arm-linux-musleabihf && mv realm-1_5-arm-linux-musleabihf /etc/realm/realm
        fi
...
Update_Shell(){
    ...
    sh_new_ver=$(wget --no-check-certificate -qO- "https://ghproxy.com/https://raw.githubusercontent.com/Anyone20/rtwo/main/realm.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
    [[ -z ${sh_new_ver} ]] && echo -e "${Error} 检测最新版本失败 !" && start_menu
    if [[ ${sh_new_ver} != ${sh_ver} ]]; then
        echo -e "最新版本为 [ ${sh_new_ver} ]，是否更新？[Y/n]"
        read -p "(默认: y):" yn
        [[ -z "${yn}" ]] && yn="y"
        if [[ ${yn} == [Yy] ]]; then
            wget -N --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/Anyone20/rtwo/main/realm.sh && chmod +x realm.sh
            echo -e "脚本已更新为最新版本[ ${sh_new_ver} ] !"
        else
            echo && echo "    已取消..." && echo
        fi
    else
        echo -e "当前已是最新版本[ ${sh_new_ver} ]，是否重新安装？[Y/n]"
        read -p "(默认: y):" yn
        [[ -z "${yn}" ]] && yn="y"
        if [[ ${yn} == [Yy] ]]; then
            `rm -rf /root/realm.sh`
            wget -N --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/Anyone20/rtwo/main/realm.sh && chmod +x realm.sh
            echo -e "脚本已重新安装，版本[ ${sh_new_ver} ] !"
        else
            echo && echo "    已取消..." && echo
        fi
    fi
...
echo "#############################################################"
echo -e "公告：$(curl -L -s --connect-timeout 3 https://ghproxy.com/https://raw.githubusercontent.com/Anyone20/rtwo/main/realm_notice)"
echo -e "
