...
# https://github.com/Anyone/rtwo

# The URL of the script is:
# https://raw.githubusercontent.com/Anyone/rtwo/main/install-release.sh

# If the script executes incorrectly, go to:
# https://github.com/Anyone/rtwo/issues
...
get_latest_version() {
  ...
  local tmp_file
  tmp_file="$(mktemp)"
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/Anyone/rtwo/releases/latest'; then
    "rm" "$tmp_file"
  ...
  if [[ -z "$RELEASE_LATEST" ]]; then
    if grep -q "API rate limit exceeded" "$tmp_file"; then
      ...
      echo "Welcome bug report:https://github.com/Anyone/rtwo/issues"
    fi
    "rm" "$tmp_file"
    exit 1
  fi
  ...
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/Anyone/rtwo/releases'; then
    "rm" "$tmp_file"
    echo 'error: Failed to get release list, please check your network.'
    exit 1
  fi
  ...
  do
    ...
    grep -q "https://github.com/Anyone/rtwo/releases/download/${releases_list[$i]}/Xray-linux-$MACHINE.zip" "$tmp_file" && break
  done
  "rm" "$tmp_file"
...
download_xray() {
  DOWNLOAD_LINK="https://github.com/Anyone/rtwo/releases/download/$INSTALL_VERSION/Xray-linux-$MACHINE.zip"
